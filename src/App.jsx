...
//Yearly Expenses at first year of retirement//////////////////////////////////////////////////////////////////////////////////
    var yearly_expenses_retirement = (this.state.expected_yearly_expenses*(1+(this.state.inflation_rate/100))**yrs_to_retire)

    const rz =  ( (1 + this.state.interest_rate/100 ) / (1+ this.state.inflation_rate/100) -1)
    const rzb = 1 + rz
    const rzn = rzb**-(this.state.expected_lifespan)
    var retirement_sum;

    if((this.state.inflation_rate===0 && this.state.interest_rate===0) || this.state.inflation_rate===this.state.interest_rate){
        retirement_sum = (yearly_expenses_retirement*this.state.expected_lifespan);
...
//Projected value of your retirement savings in N years////////////////////////////////////////////////////////////////////////////
    ...
    if(this.state.interest_rate===0){
        ...
        projected_value = annual_income_aside*((1+this.state.interest_rate/100)*(((1+this.state.interest_rate/100)**yrs_to_retire-1)/(this.state.interest_rate/100)))
        this.setState({projected_value})
    }
...
//Total Surplus or Shortfall////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    var total_funds = insurance_value + CPF_value + other_assests_value + projected_value
...
submitForm(event){
    ...
    if(isValid){
        //Hide Unhide Form
        this.state.formVisibility === true? this.setState({ formVisibility: false }):this.setState({ formVisibility:true })

        //Hide Unhide Chart
        this.state.chartVisibility === true? this.setState({ chartVisibility: false }):this.setState({ chartVisibility: true })
    }
...
validateForm(){
    ...
    if (errorCurrent_age || errorRetirement_age || errorExpected_monthly_expenses || errorExpected_lifespan || errorAnnual_income_aside || errorCPF_value || errorOther_assests_value){

        this.setState({ errorCurrent_age,
                        errorRetirement_age,
                        errorExpected_monthly_expenses,
                        errorExpected_lifespan,
                        errorAnnual_income_aside,
